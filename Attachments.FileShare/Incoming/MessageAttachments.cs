...
class MessageAttachments : IMessageAttachments
{
    ...
    public async Task ProcessStream(Func<FileStream, Task> action, CancellationToken cancellation = default)
    {
        Guard.AgainstNull(action, nameof(action));
        await persister.ProcessStream(messageId, "", action).ConfigureAwait(false);
    }

    public async Task ProcessStream(string name, Func<FileStream, Task> action, CancellationToken cancellation = default)
    {
        Guard.AgainstNull(name, nameof(name));
        Guard.AgainstNull(action, nameof(action));
    ...
    public async Task ProcessStreams(Func<string, FileStream, Task> action, CancellationToken cancellation = default)
    {
        Guard.AgainstNull(action, nameof(action));
    ...
    public async Task ProcessStreamForMessage(string messageId, Func<FileStream, Task> action)
    {
        Guard.AgainstNullOrEmpty(messageId, nameof(messageId));
        Guard.AgainstNull(action, nameof(action));
        await persister.ProcessStream(messageId, "", action).ConfigureAwait(false);
    ...
    public async Task ProcessStreamForMessage(string messageId, string name, Func<FileStream, Task> action)
    {
        Guard.AgainstNullOrEmpty(messageId, nameof(messageId));
        Guard.AgainstNull(name, nameof(name));
        Guard.AgainstNull(action, nameof(action));
    ...
    public async Task ProcessStreamsForMessage(string messageId, Func<string, FileStream, Task> action, CancellationToken cancellation = default)
    {
        Guard.AgainstNullOrEmpty(messageId, nameof(messageId));
        Guard.AgainstNull(action, nameof(action));
        await persister.ProcessStreams(messageId, action, cancellation).ConfigureAwait(false);
    ...
    public FileStream GetStreamForMessage(string messageId)
    {
        Guard.AgainstNullOrEmpty(messageId, nameof(messageId));
        return persister.GetStream(messageId, "");
    }

    public FileStream GetStreamForMessage(string messageId, string name)
    {
        Guard.AgainstNullOrEmpty(messageId, nameof(messageId));
        Guard.AgainstNull(name, nameof(name));
        return persister.GetStream(messageId, name);
