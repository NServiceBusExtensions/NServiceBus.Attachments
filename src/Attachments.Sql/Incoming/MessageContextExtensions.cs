...
public static partial class SqlAttachmentsMessageContextExtensions
{
    ...
    public static IMessageAttachments Attachments(this IMessageHandlerContext context)
    {
        ...
        {
            return new MessageAttachmentsFromTransaction(state.Transaction, state.GetConnection, context.MessageId, state.Persister, context.Cancellation);
        }

        if (state.SqlTransaction is not null)
        {
            return new MessageAttachmentsFromSqlTransaction(state.SqlTransaction, context.MessageId, state.Persister, context.Cancellation);
        }

        if (state.SqlConnection is not null)
        {
            return new MessageAttachmentsFromSqlConnection(state.SqlConnection, context.MessageId, state.Persister, context.Cancellation);
        }

        return new MessageAttachmentsFromSqlFactory(state.GetConnection, context.MessageId, state.Persister, context.Cancellation);
    }
}