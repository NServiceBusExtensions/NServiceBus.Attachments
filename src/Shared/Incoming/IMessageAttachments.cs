...
public partial interface IMessageAttachments
{
    ...
    /// </summary>
    Task CopyTo(string name, Stream target, Cancellation cancel = default);
    ...
    /// </summary>
    Task CopyTo(Stream target, Cancellation cancel = default);

    /// <summary>
    /// Process with the delegate <paramref name="action"/>, for the current message, the attachment of <paramref name="name"/>.
    /// </summary>
    Task ProcessStream(string name, Func<AttachmentStream, Cancellation, Task> action, Cancellation cancel = default);

    /// <summary>
    /// Process with the delegate <paramref name="action"/>, the attachment with the default name of <see cref="string.Empty"/>.
    /// </summary>
    Task ProcessStream(Func<AttachmentStream, Cancellation, Task> action, Cancellation cancel = default);

    /// <summary>
    /// Process with the delegate <paramref name="action"/>, all attachments for the current message.
    /// </summary>
    Task ProcessStreams(Func<AttachmentStream, Cancellation, Task> action, Cancellation cancel = default);

    /// <summary>
    /// Read all attachment metadata for the current message.
    /// </summary>
    IAsyncEnumerable<AttachmentInfo> GetMetadata(Cancellation cancel = default);
    ...
    /// </remarks>
    Task<AttachmentBytes> GetBytes(Cancellation cancel = default);
    ...
    /// This should only be used when the data size is know to be small as it causes the full size of the attachment to be allocated in memory.
    /// </remarks>
    Task<MemoryStream> GetMemoryStream(Cancellation cancel = default);

    /// <summary>
    /// Get a <see cref="byte"/> array, for the current message, the attachment of <paramref name="name"/>.
    /// </summary>
    Task<AttachmentBytes> GetBytes(string name, Cancellation cancel = default);

    /// <summary>
    /// Get a <see cref="MemoryStream"/>, for the current message, the attachment of <paramref name="name"/>.
    /// </summary>
    Task<MemoryStream> GetMemoryStream(string name, Cancellation cancel = default);
    ...
    /// This should only be used when the data size is know to be small as it causes the full size of the attachment to be allocated in memory.
    /// </remarks>
    Task<AttachmentString> GetString(Encoding? encoding = null, Cancellation cancel = default);

    /// <summary>
    /// Get a <see cref="string"/> array, for the current message, the attachment of <paramref name="name"/>.
    /// </summary>
    Task<AttachmentString> GetString(string name, Encoding? encoding = null, Cancellation cancel = default);
    ...
    /// </summary>
    Task CopyToForMessage(string messageId, string name, Stream target, Cancellation cancel = default);
    ...
    /// </summary>
    Task CopyToForMessage(string messageId, Stream target, Cancellation cancel = default);
    ...
    /// </summary>
    Task ProcessStreamForMessage(string messageId, string name, Func<AttachmentStream, Cancellation, Task> action, Cancellation cancel = default);
    ...
    /// </summary>
    Task ProcessStreamForMessage(string messageId, Func<AttachmentStream, Cancellation, Task> action, Cancellation cancel = default);

    /// <summary>
    /// Process with the delegate <paramref name="action"/>, all attachments for the message with <paramref name="messageId"/>.
    /// </summary>
    Task ProcessStreamsForMessage(string messageId, Func<AttachmentStream, Cancellation, Task> action, Cancellation cancel = default);
    ...
    /// This should only be used when the data size is know to be small as it causes the full size of the attachment to be allocated in memory.
    /// </remarks>
    Task<AttachmentBytes> GetBytesForMessage(string messageId, Cancellation cancel = default);
    ...
    /// This should only be used when the data size is know to be small as it causes the full size of the attachment to be allocated in memory.
    /// </remarks>
    Task<MemoryStream> GetMemoryStreamForMessage(string messageId, Cancellation cancel = default);
    ...
    /// This should only be used when the data size is know to be small as it causes the full size of the attachment to be allocated in memory.
    /// </remarks>
    Task<AttachmentBytes> GetBytesForMessage(string messageId, string name, Cancellation cancel = default);
    ...
    /// This should only be used when the data size is know to be small as it causes the full size of the attachment to be allocated in memory.
    /// </remarks>
    Task<MemoryStream> GetMemoryStreamForMessage(string messageId, string name, Cancellation cancel = default);
    ...
    /// This should only be used when the data size is know to be small as it causes the full size of the attachment to be allocated in memory.
    /// </remarks>
    Task<AttachmentString> GetStringForMessage(string messageId, Encoding? encoding = null, Cancellation cancel = default);
    ...
    /// This should only be used when the data size is know to be small as it causes the full size of the attachment to be allocated in memory.
    /// </remarks>
    Task<AttachmentString> GetStringForMessage(string messageId, string name, Encoding? encoding = null, Cancellation cancel = default);
}